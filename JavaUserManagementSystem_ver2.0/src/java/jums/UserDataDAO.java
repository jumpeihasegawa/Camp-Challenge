...
public class UserDataDAO {
    ...
    public ArrayList<UserDataDTO> search(UserDataDTO ud) throws SQLException {
        ...
        try {
            ...
            //searchメソッドでSELECT文を担当している部分は「空入力とワイルドカード」の数と種類を同じにしなければ複合検索ができない
            if (flag) {
            ...
            //検索で何も入力されていないときは全件表示が満たされていない「値を上書きしてしまうので他の格納の仕方にする」
            ArrayList<UserDataDTO> SeachData_Arr = new ArrayList<UserDataDTO>();
            while (rs.next()) {
                ...
                SeachData_Arr.add(resultUd);
            }
            System.out.println("search completed");

            return SeachData_Arr;
        } catch (SQLException e) {
            System.out.println(e.getMessage());
            throw new SQLException(e);
        } finally {
    ...
    public void updataByID(UserDataDTO ud) throws SQLException {
        Connection con = null;
        PreparedStatement st = null;
        try {
            con = DBManager.getConnection();
            ...
            System.out.println("updataByID completed");
        } catch (SQLException e) {
            System.out.println(e.getMessage());
            throw new SQLException(e);
        } finally {
